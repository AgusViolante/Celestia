...
void AEnemyBase::BeginPlay()
{
	...
	if (HealthComponent)
	{
		...
		HealthComponent->OnHealthChanged.AddUniqueDynamic(this, &AEnemyBase::OnHealthChangedUpdateUI);
	}

	if (OverheadWidgetComp)
	{
		if (UEnemyOverheadWidget* Widget = Cast<UEnemyOverheadWidget>(OverheadWidgetComp->GetUserWidgetObject()))
		{
			Widget->SetupWidget(EnemyName, EnemyLevel);
		}
	}
}
...
}void AEnemyBase::OnHealthChangedUpdateUI(UHealthComponent* InHealthComp, float NewHealth, float MaxHealth, float HealthDelta)
{
	if (MaxHealth > 0.f && OverheadWidgetComp)
	{
		if (UEnemyOverheadWidget* Widget = Cast<UEnemyOverheadWidget>(OverheadWidgetComp->GetUserWidgetObject()))
		{
			Widget->UpdateHealth(NewHealth / MaxHealth);
		}
	}
}
